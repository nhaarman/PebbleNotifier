...
public final class PebbleKit {
    ...
    public static abstract class PebbleDataLogReceiver extends BroadcastReceiver {
        ...
        /**
         * Handle an long data unit that was logged the watch and broadcast by pebble.apk.
         *
         * @param context   The BroadcastReceiver's context.
        ...
        public void receiveData(final Context context, UUID logUuid,
                                ...
                                final long data) {
            throw new UnsupportedOperationException("long handler not implemented");
        ...
        private void handleReceiveDataIntent(final Context context, final Intent intent, final UUID logUuid,
            ...
            switch (type) {
                case BYTES:
                    ...
                    break;
                case UINT:
                    long uint = (long) intent.getLongExtra(PBL_DATA_OBJECT, -1);
                    if (uint == -1) {
                        throw new IllegalArgumentException();
                    }
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            try {
                ...
                timestamp = (long) intent.getLongExtra(DATA_LOG_TIMESTAMP, -1);
                if (timestamp == -1) {
                    throw new IllegalArgumentException();
                }

                tag = (long) intent.getLongExtra(DATA_LOG_TAG, -1);
                if (tag == -1) {
                    throw new IllegalArgumentException();
                }
